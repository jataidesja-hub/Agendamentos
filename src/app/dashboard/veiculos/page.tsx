...
export default function VeiculosPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const { error } = await supabase.from('veiculos_frota').insert({ placa, modelo });
  ...
  return (
    <div className="p-4 md:p-8 w-full max-w-5xl mx-auto flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {veiculos.map(v => (
          <div key={v.id} className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-md border border-gray-100 dark:border-gray-700/50 rounded-2xl p-5 shadow-lg flex items-center justify-between group hover:border-blue-500/50 transition-colors">
            <div>
              <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">{v.modelo}</p>
              <h3 className="text-xl font-black text-gray-900 dark:text-white tracking-widest">{v.placa}</h3>
            </div>
