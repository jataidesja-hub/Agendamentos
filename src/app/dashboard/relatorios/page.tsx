    ...
    <div className="h-full flex flex-col p-6 gap-6">
      ...
      <div className="bg-white/60 dark:bg-gray-800/60 backdrop-blur-xl rounded-[2.5rem] p-8 border border-white/40 shadow-xl w-full flex items-center justify-between">
        ...
        <div className="hidden md:flex gap-2 bg-gray-100/50 dark:bg-gray-900/50 p-1.5 rounded-3xl border border-gray-200/50 dark:border-gray-700/50">
          {abasPermitidas.includes('projetos') && (
            <button
              onClick={() => setActiveTab('projetos')}
              className={`flex items-center gap-2 px-6 py-3 rounded-2.5xl text-xs font-black tracking-widest transition-all ${
                activeTab === 'projetos' ? 'bg-[#0b7336] text-white shadow-lg shadow-green-500/20' : 'text-gray-500 hover:text-green-600'
              }`}
            >
              <TruckIcon className="w-4 h-4" /> PROJETOS
            </button>
          )}
          {abasPermitidas.includes('postos') && (
            <button
              onClick={() => setActiveTab('postos')}
              className={`flex items-center gap-2 px-6 py-3 rounded-2.5xl text-xs font-black tracking-widest transition-all ${
                activeTab === 'postos' ? 'bg-blue-600 text-white shadow-lg shadow-blue-500/20' : 'text-gray-500 hover:text-blue-600'
              }`}
            >
              <BuildingStorefrontIcon className="w-4 h-4" /> POSTOS
            </button>
          )}
          {abasPermitidas.includes('gestores') && (
            <button
              onClick={() => setActiveTab('gestores')}
              className={`flex items-center gap-2 px-6 py-3 rounded-2.5xl text-xs font-black tracking-widest transition-all ${
                activeTab === 'gestores' ? 'bg-orange-600 text-white shadow-lg shadow-orange-500/20' : 'text-gray-500 hover:text-orange-600'
              }`}
            >
